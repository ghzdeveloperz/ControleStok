...
export const Alertas: React.FC = () => {
  ...
  // Produtos com estoque baixo, usando minStock de cada produto
  const lowProducts: Product[] = products
    .filter((p) => {
      const qty = Number(p.quantity ?? 0);
      const minStock = Number(p.minStock ?? 10); // fallback caso não tenha

      // estoque baixo = maior que 0, mas menor/igual ao mínimo
